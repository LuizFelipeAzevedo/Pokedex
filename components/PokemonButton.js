...
const PokemonButton = ({ id, shadowed = false }) => {
    ...
    return (
        ...
        <Pressable style={styles.container} onPress={() => navigation.navigate('Pokemon', {id})}>
            <Text style={styles.test}>{`#${idString}`}</Text> 
            <Image source={imageSrc} style={{...styles.image, tintColor: shadowed ? '#555' : undefined}}/>
            <Text style={styles.text}>{pokedex[id-1].name.english}</Text>
        </Pressable >
    )
}
